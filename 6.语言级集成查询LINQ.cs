...
//A: Single()方法。Where()方法返回的是IEnumerable<T>列表，如果只用f反回一个对象的话就用Single()方法，使用方法与Where()差不多，都是传入一个签
  //名为(Book book)、返回值为bool的方法或等价的匿名方法(Func<T, bool>委托类型)。但是如果查找对象中没有满足匿名方法条件的对象时，控制台会抛出异常。
